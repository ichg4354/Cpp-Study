...
void fileWrite(int num, double rate[], int maxScore[], string name[], int mid[], int fnl[], int hw[], int par[])
{
     string title;
     ...
     for (int i = 0; i < num; i++)
     {
          writeStudentScore(outputStream, name[i], mid[i], fnl[i], par[i], hw[i]);
     }
...
bool fileRead(double rate[], int maxScore[], string name[], int mid[], int fnl[], int par[], int hw[])
{
     string title;
     bool flag = true;
     int num = 0;
     ...
     while (flag)
     {
          flag = getScore(inputStream, name[num], mid[num], fnl[num], par[num], hw[num]);
          num++;
     }
}
...
int main()
{
     ...
     while (1)
     {
          ...
          {
               fileWrite(3, weights, maxScores, name, mid, fnl, hw, par);
          }
          else if (input == 6)
          {
               fileRead(weights, maxScores, name, mid, fnl, par, hw);
          }
          else if (input == 7)
          {
